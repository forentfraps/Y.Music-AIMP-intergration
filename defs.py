...
class YM:
    ...
    def download(self, trackId, dir: str):
        ...
        try:
            ...
            info = req['download-info']
            sign = md5(('XGRlBW9FXlekgbPrRHuSiA' + info['path'][1::] + info['s']).encode('utf-8')).hexdigest()
    ...
    def like(self, trackId: int, remove = False):
        ...
        resp = self.post(link, {"track-ids": trackId})
        self.download(trackId, "./Y/likes")
    def fetch_daily_playlist(self):
        #['Premiere', 'Playlist of the Day'] dirs
